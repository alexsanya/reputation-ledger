...
export async function commitSuccess(ctx: TestContext) {
    ...
    let tx = new anchor.web3.Transaction()
        ...
        try {
            ...
            assert.equal(order.priceValidUntil.toString(), orderData.price_valid_until.toString());
...
export async function commitWrongInstruction(ctx: TestContext) {
    ...
    let tx = new anchor.web3.Transaction()
        ...
        try {
            ...
            assert.isTrue(error.logs.some(log => log.includes("Error Code: InvalidProgramId")));
        }
}
...
export async function commitWrongSignature(ctx: TestContext) {
    ...
    let tx = new anchor.web3.Transaction()
        ...
        try {
            ...
            assert.isDefined(error.logs);
            assert.isTrue(error.logs.some(log => log.includes("Error Code: InvalidSignature")));
        }
}
